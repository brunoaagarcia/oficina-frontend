...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 flex h-14 items-stretch border-t border-line bg-surface sm:hidden">
      ...
      {usuario?.papel === 'MODERADOR' && (
        <Link to="/clientes" className={`flex flex-1 flex-col items-center justify-center gap-0.5 ${ativo('/clientes')}`}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" />
            <circle cx="12" cy="7" r="4" />
          </svg>
          <span className="text-[10px] font-medium leading-none">Clientes</span>
        </Link>
      )}
